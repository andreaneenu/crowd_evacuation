...
def leap_frog(y0, v0, f, e_0, N_steps, dt, room, sim_class):
    food_dist_rad = sim_class.collection_radius
    tmp = 0
    deadstillnow = 0
    agents_escaped = np.zeros(N_steps)
    dead = np.zeros(N_steps)
    v_des = 1.5
    y = np.zeros((y0.shape[0], y0.shape[1], N_steps))
    v = np.zeros((y0.shape[0], y0.shape[1], N_steps))
    a = np.zeros((y0.shape[0], y0.shape[1], N_steps))
    ...
    for k in range(N_steps-1):
        # print(100*k/N_steps, '% done.')
        y[:, :, k+1] = y[:, :, k] + dt*v[:, :, k]
        a[:, :, k], pressure = f(y[:, :, k], v[:, :, k], status[:, k],sim_class=sim_class)
        v[:, :, k+1] = v[:, :, k] + dt * \
            f(y[:, :, k+1], v[:, :, k] + dt*a[:, :, k], status[:, k],sim_class=sim_class)[0]
        
        if(k==0.7/sim_class.tau):
            status[:,k]=1
        ...
        for i in range(y.shape[1]):
            ...
            if status[i][k+1] == 3:
                if counter[i] == 0:
                    status[i][k+1] = 2
                    v[:, i, k+1] = v_des*e_0(y[:, i, k+1], i, status[i][k+1])
                    tmp += 1
                counter[i] = counter[i]-1
            if distance < food_dist_rad and status[i][k+1] == 1:
                # we have to use position of door here instead of (0,5)
                # y[:, i, k+1] = 10**6 * np.random.rand(2)
                status[i][k+1] = 3
                v[:, i, k+1] = v_des*e_0(y[:, i, k+1], i, status[i][k+1])
        ...
        if 100*k/N_steps >= lst+10:
            lst += 10
            print(lst, '%', ' done')
        agents_escaped[k+1] = tmp
        dead[k+1] = deadstillnow

    return y, agents_escaped, a, status, dead
...
#     parameters = np.sort(np.random.uniform(
#         low=0.0, high=dt, size=(iterations-1, samples)), axis=0)
...
#     for k in range(N_steps-1):
#         y[:, :, k+1], v[:, :, k +
#                         1] = mcstep(y[:, :, k], v[:, :, k], f, 0.0, 0.5, room, samples)
#         a[:, :, k+1] = f(y[:, :, k+1], v[:, :, k+1])
...
# def odestep(y0, v0, f, t, dt, room, dtmin=0.0001, tol=0.00001, maxiter=10):
#     ytemp = y0
#     vtemp = v0
#     it = 0
#     step = t
...
#         k6 = f(ytemp + dx*v6, vtemp + dx *
#                (a61*k1 + a62*k2 + a63*k3 + a64*k4 + a65*k5))

#         v7 = a71*v1 + a73*v3 + a74*v4 + a75*v5 + a76*v6
#         k7 = f(ytemp + dx*v7, vtemp + dx *
#                (a71*k1 + a73*k3 + a74*k4 + a75*k5 + a76*k6))

#         # Error Control
#         error = np.linalg.norm((a71-a81)*k1 + (a73-a83)*k3 +
#                                (a74-a84)*k4 + (a75-a85)*k5 + (a76-a86)*k6 - a87*k7)
...
#     for k in range(N_steps-1):
#         y[:, :, k+1], v[:, :, k +
#                         1] = odestep(y[:, :, k], v[:, :, k], f, 0.0, 0.5, room)
#         a[:, :, k+1] = f(y[:, :, k+1], v[:, :, k+1])

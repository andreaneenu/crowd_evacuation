...
class Diff_Equ:
    def __init__(self, num_individuals, L, tau, room, radii, weights):
        ...
        # position of corner points of the walls
    ...
    def agent(self, i, j, r, v):
        ...
        n = (r[:, i] - r[:, j])/np.sqrt(np.square(d)+0.001)
        t = np.array([-n[1], n[0]])
        temp = v[:, j] - v[:, i]
        dv_t = temp.dot(t)
        return d, n, t, dv_t
    ...
    def f_ij(self, i, j, r, v, status):
        ...
        rad_ij = self.rad(i, j)
        # a = (status[i]!=0)*self.A * np.exp((rad_ij - d) / self.B) + \
        #     self.k * self.g(rad_ij - d)
        a = self.A * np.exp((rad_ij - d) / self.B) + \
            self.k * self.g(rad_ij - d)
        b = self.kap * self.g(rad_ij - d) * dv_t
        # if np.linalg.norm(a)>3200:
        #     print("dead?")
        return a * n + b * t
    ...
    def f_iW(self, i, j, r, v):
        ...
        a = self.A * np.exp((self.radii[i] - d) / self.B) + \
            self.k * self.g(self.radii[i] - d)
    ...
    def direction(self, i, j, r):
        ...
        """
        wall = self.walls[j, :, :]
        wall_norm = (wall[0, :]-wall[1, :]) / \
            np.linalg.norm(wall[0, :]-wall[1, :])
    ...
    def e_1(self, r_i, temp_wall, i, j):
        ...
        wall_norm = (temp_wall[0, :]-temp_wall[1, :]) / \
            np.linalg.norm(temp_wall[0, :]-temp_wall[1, :])
        check_close_corner = np.zeros((all_walls.shape[0], 2))
        point = self.seg_intersect(
            r_i, self.r_D, temp_wall[0, :], temp_wall[1, :])
        t_or_f = self.is_between(temp_wall[0, :], temp_wall[1, :], point)

        for k in range(all_walls.shape[0]):
            if k == j:
                continue
            check_close_corner[k, 0] = self.is_between(
                all_walls[k, 0, :], all_walls[k, 1, :], temp_wall[0, :])
            check_close_corner[k, 1] = self.is_between(
                all_walls[k, 0, :], all_walls[k, 1, :], temp_wall[1, :])
        ...
        if np.sum(check_close_corner) == 1:
            for k in range(all_walls.shape[0]):
                if (check_close_corner[k, 0] == 1 and check_close_corner[k, 1] == 0) and self.is_between(all_walls[k, 0, :], all_walls[k, 1, :], self.seg_intersect(r_i, self.r_D, all_walls[k, 0, :], all_walls[k, 1, :])) == 1:
                    e = self.nearest_path(
                        temp_wall, t_or_f, point, wall_norm, r_i, i)
                    break
    ...
    def e_t(self, r, status):
        ...
        if self.wallshere == False:
            for i in range(self.N):
                e_temp[:, i] = self.e_0(r[:, i], i, status[i])
        else:
            for i in range(self.N):
    ...
    def f_ag(self, r, v, status):
        ...
        for i in range(self.N-1):
            for j in range(self.N-1-i):
                # if i==j:
                #     continue
                # fij[:, i, j] = self.f_ij(i, j, r, v,status) # force on i by i+j+1
                # force on i by i+j+1
                fij[:, i, j+i+1] = self.f_ij(i, j+i+1, r, v, status)

                # self.pressure[i]+=np.linalg.norm(fij[:, i, j]) # scaler force on i by i+j+1 adding to pressure
                
                # scaler force on i by i+j+1 adding to pressure
                self.pressure[i] += np.linalg.norm(fij[:, i, j+i+1])
                
                fij[:, j+i+1, i] = -fij[:, i, j+i+1]  # force on i+j+1 by i

                # self.pressure[j+i+1]+=np.linalg.norm(fij[:, i, j+i+1]) # scaler force on i+j+1 by i adding to pressure
        f_agent = np.sum(fij, 2)
        return f_agent

    def f_i_circ(self, i, r, v, circ_wall):
        """ returns an 1D-array, the frocevector between agent i and j
            parameters: ``i``,``j``: integers (agent i and j)
                        ``r``,``v``: 2D-array (position and velocity of all agents)
        """
        d, n, t, dv_t = self.agent_circ(i, r, v, circ_wall)
        rad_ij = self.radii[i]+circ_wall.radius
        a = self.A * np.exp((rad_ij - d) / self.B) + \
            self.k * self.g(rad_ij - d)
        b = self.kap * self.g(rad_ij - d) * dv_t
        # if np.linalg.norm(a)>3200:
    ...
    def agent_circ(self, i, r, v, circ_wall):
        """ returns:    ``d``      : float (distance between agend i and j)
                        ``n``,``t``: 1D-array (normalized vector pointing from
                                     agent i to j and it's tangential vector)
                        ``dv_t``   : float (difference in velocity)
            parameters: ``i``,``j``: integers (agent i and j)
                        ``r``,``v``: 2D-array (position and velocity of all agents)
        """
        d = np.linalg.norm(r[:, i] - circ_wall.center)
        n = (r[:, i] - circ_wall.center)/np.sqrt(np.square(d)+0.001)
        t = np.array([-n[1], n[0]])
        temp = - v[:, i]
        dv_t = temp.dot(t)
        return d, n, t, dv_t

    def f_circwalls(self, r, v, circular_Obstacles):
        f_agent = np.zeros((2, self.N))
        for i in range(self.N):
            for j in range(len(circular_Obstacles)):
                tmp = self.f_i_circ(i, r, v, circular_Obstacles[j])
                f_agent[:, i] += tmp
                # scaler force on i by i+j+1 adding to pressure
                self.pressure[i] += np.linalg.norm(tmp)
        return f_agent
    ...
    def f_wa(self, r, v):
        ...
        for i in range(self.N):
            for j in range(self.numwalls):
                ...
                # scaler force on i by wall j adding to pressure array
                self.pressure[i] += np.linalg.norm(f_wall[:, i])
        return f_wall

    # The diff_equation of our problem
    # Calculates the accelaration of each agent
    def f(self, r, v, status, sim_class):
        """ returns a 2D-array 
        v = the velocity at time t
        r = the position at time t"""
        self.pressure = np.zeros(status.shape[0])
        ...
        for i in range(status.shape[0]):
            self.pressure[i] /= self.radii[i]
        return acc, self.pressure

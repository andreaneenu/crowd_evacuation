...
class Room:
    def __init__(self, room, room_size):
        ...
        if room == "square":
            ...
            self.destination = np.array([[0.0, room_size/2]])   # destination the agenst want to go
            ...
            # self.walls = np.array([[[0, 0], [0, room_size]],                    # wall 1  
            #               [[0, room_size], [room_size, room_size]],             # wall 2
            #               [[room_size, room_size], [room_size, 0]],             # wall 3
            #               [[room_size, 0], [0, 0]]])                            # wall 4
            center=[5,room_size/2-0.3]
            sidelen=0.5
            # square=self.Make_Square(center=center,sidelen=sidelen)
            # rect=self.Make_Rect(center=center,verlen=5,breath=0.5)
            self.walls = np.array([[[0, 0], [0, room_size]]])                     # wall 1
            # self.walls=np.append(self.walls,rect,axis=0)
            self.num_walls = len(self.walls)
            # agents spawn with x and y position between 1 and (room_size-1) 
            self.spawn_zone = np.array([[ 1,room_size-1], [1, room_size-1]])
    ...
    def Make_Square(self,center,sidelen):
        return [[[center[0]+sidelen/2,center[1]+sidelen/2] , [center[0]-sidelen/2,center[1]+sidelen/2]],
                [[center[0]-sidelen/2,center[1]+sidelen/2] , [center[0]-sidelen/2,center[1]-sidelen/2]],
                [[center[0]-sidelen/2,center[1]-sidelen/2] , [center[0]+sidelen/2,center[1]-sidelen/2]],
                [[center[0]+sidelen/2,center[1]-sidelen/2] , [center[0]+sidelen/2,center[1]+sidelen/2]]]
    
    def Make_Rect(self,center,verlen,breath):
        return [[[center[0]+breath/2,center[1]-verlen/2] , [center[0]+breath/2,center[1]+verlen/2]],
                [[center[0]+breath/2,center[1]+verlen/2] , [center[0]-breath/2,center[1]+verlen/2]],
                [[center[0]-breath/2,center[1]+verlen/2] , [center[0]-breath/2,center[1]-verlen/2]],
                [[center[0]-breath/2,center[1]-verlen/2] , [center[0]+breath/2,center[1]-verlen/2]]]
